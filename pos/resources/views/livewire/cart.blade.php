    ...
    <div class="col-md-8">
        <div class="card">
            ...
            <div class="row">
                    ...
                    <div class="col-md-7"><input wire:model="search" type="text" class="fore-control" placeholder="Search Categories..."></div>
                </div>
            </div>
            <div class="card-body">
                    ...
                    @forelse($products as $product)
                        <div class="col-md-3 mb-3" :key="{{$product->id}}">
                            <div class="card" wire:click="addItem({{$product->id}})" style="cursor:pointer">
                                ...
                                <button class="btn btn-primary btn-sm" style="position:absolute;top:0;left:0;padding: 10px 15px"><i class="fas fa-cart-plus fa-lg"></i></button>
    ...
    <div class="col-md-4">
        ...
        <div class="card mt-4">
            <div class="card-body">
                ...
                <h5 class="font-weight-bold">Point (Membership):  {{ $summary['point'] }} </h5>
                ...
                <div class="row mt-4">
                    <div class="col-sm-6">
                        <button wire:click="enablePoint" class="btn btn-primary btn-block btn-sm">Add Point</button>
                    </div>
                    <div class="col-sm-6">
                        <button wire:click="disablePoint" class="btn btn-danger btn-block btn-sm mb-3">Remove Point</button>
                    </div>      
                </div>
            <div>
                <h7 class="font-weight-bold">Payment Method</h7>
            <select wire:model="method" class="form-control mb-2">
                    <option value="1"> Cash</option>
                    <option value="2"> Point</option>
            </select>
            </div>
            <div>
            <h7 class="font-weight-bold">Membership</h7>
                <select wire:model="Customer" class="form-control mb-2">
                    <option></option>
                    @foreach ($customer as $items)
                    <option value="{{$items->id}}">{{$items->name}}      |Total Point {{$items->point}} </option>
                    @endforeach
                </select>
                <input type="number" wire:model="payment" class="form-control mb-3" id="payment" placeholder="Input customer payment amount">
